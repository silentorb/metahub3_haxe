...
class Coder {
  ...
  public function convert_expression(source:Dynamic, scope:Scope):Expression {
    ...
    switch(source.type) {
      ...
      //case 'set_property':
        //return set_property(source, scope);
			case 'new_scope':
  ...
  function create_block(source:Dynamic, scope:Scope):Expression {
    ...
    for (e in fields) {
      ...
      block.add(convert_statement(child, scope));
    }
  ...
  function function_expression(source:Dynamic, scope:Scope, name:String, reference:Expression = null):Expression {
    ...
    //var inputs = Lambda.array(Lambda.map(expressions, function(e) return convert_expression(e, scope, type)));
  ...
  static function get_type(value:Dynamic):Signature {
    if (Std.is(value, Int)) {
			var result = new Signature(Kind.unknown);
			result.is_numeric = 1;
      return result;
		}
	...
	function new_scope(source:Dynamic, scope:Scope):Expression {
		...
		var expression:Expression = null;
		var new_scope_definition = new Scope(scope);
