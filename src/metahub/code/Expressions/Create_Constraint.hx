	...
	public function to_port(scope:Scope, old_group:Group, signature_node:Type_Signature):General_Port {
		...
		if (is_back_referencing || scope.definition.is_particular_node || inside_back_reference) {
			...
			if (operator == "equals") {
				...
				var function_info = Function_Call.get_function_info(operator, scope.hub);
				var function_node = Function_Call.instantiate(function_info, function_signature, group, scope.hub, false);
		...
		if (operator == "equals") {
			...
			var function_info = Function_Call.get_function_info(operator, scope.hub);
			assignment = Function_Call.instantiate(function_info, [ reference.get_type(type_unknown)[0], expression.get_type(type_unknown)[0] ], group, scope.hub, true);
			group.get_port(1).connect(assignment.get_port(0));
			assignment.get_port(1).connect(target);
			assignment.get_port(2).connect(source);
		}
